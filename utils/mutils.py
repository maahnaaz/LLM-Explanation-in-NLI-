...
def get_dev_test_with_expl(data_path, data_type, preproc, min_freq):
    # si.test,train,dev from explanation files are created.     
...
def bleu_prediction(pred_file, data):
    '''
    computes the BLEU score for a set of predictions (pred_file) given a reference dataset (data)
    '''
    candidates = []
    references = []
...
def bleu_inter_annotations_expl3_wrt_12(data):
    '''
    computes the BLEU score for explanations in expl_3 with respect to the combined explanations in expl_1 and expl_2
    '''
    candidates = []
    references = []
...
def pretty_duration(seconds):
    ...
    Output: a string representing the duration in the format "hours:minutes:seconds
    '''
...
def get_key_from_val(val, dic_labels):
    '''
    Inputs: val: The value for which you want to find the corresponding key.
            dic_labels: A dictionary where keys and values are paired.
    Output: the matched key
    '''
...
def get_keys_from_vals(vals, dic_labels):
    '''
    Like the above function except that vals here are tensors.
    '''
...
def get_sentence_from_indices(dictionary, tensor_indices):
    '''
    convert a tensor of indices into a human-readable sentence using a given dictionary.
    '''
    s = ''
...
def get_bow_expl_from_indices(out_vocab, tensor_indices):
    '''
    Inputs: out_vocab: A vocab_dict, keys=indices, values=words.
            tensor_indices: A tensor of indices to retrieve the corresponding words from the out_vocab  and concatenate them into BoW.
    '''
    s = ''
...
def bow_correct(pred, tgt):
    '''
    calculates the number of correct predictions in a BoW
    '''
...
def bow_correct_per_row(pred, tgt):
    '''
    calculates the number of correct predictions per row in a BoW
    '''
    assert_same_dims(pred, tgt)
    assert len(pred.size()) == 2, str(len(pred.size()))
...
def assert_sizes(t, dims, sizes):
    '''
    asserts if the dimensions and sizes of a tensor (t) match the expected dimensions (dims) and sizes (sizes).
    '''
...
def assert_same_dims(x, y):
    '''
    asserts if two tensors have same dim and size.
    '''
