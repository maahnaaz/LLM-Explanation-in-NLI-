...
def get_batch(batch, word_vec):
    ...
    '''
    Converts a batch of sentences into a tensor of word embeddings.
    Returns a tuple: (torch.FloatTensor(T, bs, 300)
    '''
...
def get_target_expl_batch(batch, word_index):
    '''
    Prepares target explanation batches for training.
    Returns a tuple: (torch.LongTensor, array_sentence_lengths)
    '''
    lengths = np.array([len(x) for x in batch])
    max_len = np.max(lengths)
...
def build_vocab(sentences, glove_path):
    '''
    Builds a vocabulary using GloVe vectors and prints the vocabulary size.
    Returns the word vectors dictionary.
    '''
