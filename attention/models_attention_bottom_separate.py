...
class AttentionDecoder(nn.Module):
    '''
    This class represents an attention-based decoder for a sequence-to-sequence model.
    '''
        ...
        if mode == "teacher":
            for t_dec in range(current_T_dec):
                ...
                assert_sizes(att_applied_perm_2, 3, [
                             1, batch_size, self.att_hid_dim])
                # Added by Mahnaaz
                expl_t_dec = expl[t_dec].unsqueeze(0).to(device)
                att_applied_perm_1 = att_applied_perm_1.to(device)
                att_applied_perm_2 = att_applied_perm_2.to(device)
                input_dec = torch.cat([expl_t_dec, att_applied_perm_1, att_applied_perm_2], 2)
...
class BLSTMEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        # changed by Mahnaaz
        idx_sort = idx_sort.to(sent.device)
        sent = sent.index_select(1, Variable(idx_sort))

        # Handling padding in Recurrent Networks
        sent_packed = nn.utils.rnn.pack_padded_sequence(sent, sent_len)
        self.enc_lstm.flatten_parameters()
        # Commented and changed by Mahnaaz sent_output = self.enc_lstm(sent_packed)[0]  # seqlen x batch x 2*nhid
        sent_output = self.enc_lstm(sent_packed.to(device))[0]
        ...
        if sent_output.size(0) < self.max_T_encoder:
            pad_tensor = Variable(torch.zeros(
                self.max_T_encoder - sent_output.size(0), sent_output.size(1), sent_output.size(2)))
            sent_output = sent_output.to(device)
