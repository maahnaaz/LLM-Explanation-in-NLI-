...
parser.add_argument("--esnli_path", type=str,
                    default='../dataset/eSNLI/', help="eSNLI data path") # Changed by Mahnaaz, dir dataset from old
...
parser.add_argument("--n_epochs", type=int, default=20) # Changed by Mahnaaz
...
# Added by Mahnaaz
parser.add_argument("--train_snli_classif", type=bool, default=False)
parser.add_argument("--use_prototype_senteval", type=bool, default=False)
parser.add_argument("--do_image_caption", type=bool, default=False)
...
current_run_dir = params.results_dir + "/" + \
    time.strftime("%H.%M_%d.%m.%y") + params.save_title
...
# Changed by Mahnaaz from eval_sent_embeddings_labels_in_expl.py, name too long
# Changed by Mahnaaz from eval_sent_embeddings_labels_in_expl.py, name too long
copy2('copy_eval_sent.py', current_run_dir)
...
# Added by Mahnaaz
train['s1'] = train['s1'][:1000]
train['s2'] = train['s2'][:1000]
train['expl_1'] = train['expl_1'][:1000]
"""
CREATE MODEL
"""
# model config
...
def trainepoch(epoch):
    ...
    for stidx in range(0, len(s1), params.batch_size):
        ...
        tgt_label_batch = Variable(torch.LongTensor(
            label[stidx:stidx + params.batch_size]).to(device))
...
def evaluate_dev(epoch):
    ...
    for i in range(0, len(s1), params.eval_batch_size):
        ...
        tgt_label_batch = Variable(torch.LongTensor(
            label[i:i + params.eval_batch_size]).to(device))
    ...
    if eval_acc > val_acc_best or dev_ppl[-1] < val_ppl_best:
        ...
        # if alpha = 0 (EXPL_ONLY) we only save the model if decrease in PPL
            ...
            current_best_model_path = best_model_prefix + \
                'Best_Model.pt'.format(dev_ppl[-1], epoch)
            torch.save(esnli_net, current_best_model_path)
            for f in glob.glob(best_model_prefix + '*'):
                if f != current_best_model_path:
                    os.remove(f)
