...
import sys
sys.path.append("..")
from utils.mutils import get_keys_from_vals, assert_sizes, get_key_from_val
    ...
    def forward(self, expl, s1_embed, s2_embed, mode, classif_lbl):
        ...
        if self.use_init:
            ...
            init_0 = Variable(torch.zeros(self.n_layers_dec,
                                          batch_size, self.dec_rnn_dim).to(device))
        ...
        if mode == "teacher":
            ...
            while t < self.max_T_decoder and not array_all_true(finished):
                ...
                word_embed = Variable(word_embed.to(device))
