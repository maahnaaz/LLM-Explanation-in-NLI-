...
class BLSTMEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        idx_sort = torch.from_numpy(idx_sort).to(device) if self.is_cuda() \
            else torch.from_numpy(idx_sort)
        idx_sort = idx_sort.to(sent.device) # Made by Mahnaaz
        sent = sent.index_select(1, Variable(idx_sort))
        sent_len = list(sent_len)  # Made by Mahnaaz
        ...
        # Un-sort by length
        idx_unsort = torch.from_numpy(idx_unsort).to(device) if self.is_cuda() \
            else torch.from_numpy(idx_unsort)
	# Commented and changed by Mahnaaz 
        idx_unsort = idx_unsort.to(device)
        sent_output = sent_output.index_select(1, idx_unsort)
        sent_output = sent_output.to(device)
        # sent_len = np.array(sent_len)[idx_unsort]
        sent_len = np.array(sent_len)[idx_unsort.cpu()]

        # Pooling
        if self.pool_type == "mean":
            sent_len = Variable(torch.FloatTensor(
                sent_len)).unsqueeze(1).to(device)
