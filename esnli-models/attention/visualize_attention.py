...
from utils.mutils import remove_file, get_key_from_val, get_sentence_from_indices, bleu_prediction
import csv
from train_attention_bottom import criterion_expl   # Added by Mahnaz
import math  # Added by Mahnaz
...
parser.add_argument("--print_every", type=int, default=500) # Added by Mahnaz
...
def visualize_attention(esnli_net, dataset, data, word_vec, word_index, current_run_dir, batch_size=1):
    ...
    if dataset == 'snli_dev':
        bleu_score = bleu_prediction(expl_csv, snli_dev_no_unk, False, False)       # Not Found, I think they are proccessed files without unkonwn token.
    else:
        bleu_score = bleu_prediction(expl_csv, snli_test_no_unk, False, False)      # Not Found, I think they are proccessed files without unkonwn token.
...
state_att = torch.load(os.path.join(
    parser.directory, parser.state_path))     # MAhnaz changed eval_params to parser
...
params.current_run_dir = parser.directory
