...
parser.add_argument("--avg_every", type=int, default=2)
...
parser.add_argument("--state_path_expl_to_labels", type=str,
                    default='state_dict_best_devacc__devACC96.799__epoch_16_model.pt')
...
params.gpu = torch.cuda.current_device()
print(params.gpu)
...
current_run_dir = params.results_dir + "/" + \
    time.strftime("%H.%M_%d.%m.%y") + params.save_title     # Changed by Mahnaz
...
# defualt directory given to the code, commented by Mahnaaz
# print(os.path.join(params.directory_expl_to_labels))
# copy2(os.path.join(params.directory_expl_to_labels, "models_expl_to_labels.py"), '.')
copy2(os.path.join("../expl_to_label/", "models_expl_to_labels.py"), '.') # Added by Behnam
...
# Edited by Mahnaaz for numpy version
for split in ['s1', 's2', 'expl_1']:
    train[split] = np.asarray([['<s>'] + [word for word in sent.split()
                                        if word in word_vec] + ['</s>'] for sent in train[split]], dtype="object")
...
# Mahnaaz
"""
train['s1'] = train['s1'][:10]
train['s2'] = train['s2'][:10]
train['expl_1'] = train['expl_1'][:10]
"""
train['s1'] = train['s1']
train['s2'] = train['s2']
train['expl_1'] = train['expl_1']
...
def trainepoch(epoch):
    ...
    for stidx in range(0, len(s1), params.batch_size):
        ...
        s1_batch, s2_batch, input_expl_batch = Variable(s1_batch.to(device)), Variable(
            s2_batch.to(device)), Variable(input_expl_batch.to(device))
        # Commented by Mahnaaz Variable is deprecated 
...
def evaluate_dev(epoch):
    ...
    s1 = snli_dev['s1'][:10]
    s2 = snli_dev['s2'][:10]
    expl_1 = snli_dev['expl_1'][:10]
    expl_2 = snli_dev['expl_2'][:10]
    expl_3 = snli_dev['expl_3'][:10]
    label = snli_dev['label'][:10]
    '''
    s1 = snli_dev['s1']
    s2 = snli_dev['s2']
    ...
    if dev_ppl[-1] < val_ppl_best:
        ...
        current_best_model_path = best_model_prefix + 'Best_Model.pt'
        #    r'_devPPL{0:.3f}__epoch_{1}_model.pt'.format(dev_ppl[-1], epoch)
        ...
        current_best_model_state_dict_path = best_state_dict_prefix + 'Best_Model.pt'
        #    '_devPPL{0_.3f}__epoch_{1}_model.pt'.format(dev_ppl[-1], epoch)
...
state_expl_to_labels = torch.load(os.path.join(
   "../expl_to_label/results_expl_to_labels/18.24_24.02.24_sgd,lr=0.1_Enc2048_bs256_gpu0_LRdecay0.99_MAXpool", params.state_path_expl_to_labels)) # Added by Behnam
...
expl_net = ExplToLabelsNet(model_config_expl_to_label)
